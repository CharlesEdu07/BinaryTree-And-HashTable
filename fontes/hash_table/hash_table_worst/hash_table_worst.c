...
int* generate_divisible_array(int size, int divisor) {
...
int main(int argc, char **argv) {
    ...
    int* divisible_array = generate_divisible_array(n, 1024);
    
    srand(time(NULL));

    for (int i = 0; i < n; i++) {
        insert(table, divisible_array[i]);
    }

    int search_index = rand() % n;

    clock_gettime(CLOCK_MONOTONIC, &start);
    int found = search(table, divisible_array[search_index]);
